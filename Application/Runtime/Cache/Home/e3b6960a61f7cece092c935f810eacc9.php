...
<div class="page-container">
	...
	<form action="/Home/Auth/Auth_pldel" method="post"  enctype="multipart/form-data" >
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
<!-- 	<span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>  -->
	<span class="l"><input type="submit" value="&#xe6e2;批量删除" class="btn btn-danger radius" >
	<a href="javascript:;" onclick="member_add('添加权限','/Home/Auth/addAuth','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加权限</a></span> 
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
		<?php if(is_array($authlist)): $i = 0; $__LIST__ = $authlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				<td><input type="checkbox" value="<?php echo ($vo["auth_id"]); ?>" name="autharray[]"></td>
				<td><?php echo ($i); ?></td>
				...
				<td hidden="hidden"></td>
				<td class="td-manage">
				<a title="修改权限" href="javascript:;" onclick="member_edit('修改权限','/Home/Auth/updateAuth/auth_id/<?php echo ($vo["auth_id"]); ?>','4','','510')" class="ml-5" style="text-decoration:none">
					<i class="Hui-iconfont">&#xe6df;</i></a> 
					<a title="删除" href="/Home/Auth/Auth_del/auth_id/<?php echo ($vo["auth_id"]); ?>" class="ml-5" style="text-decoration:none" onclick="return confirm('确定要删除吗？')">
					<i class="Hui-iconfont">&#xe6e2;</i></a>
				</td>
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '/Home/Auth/members_del/id/<?php echo ($vo["member_id"]); ?>',
			dataType: 'json',
			success: function(data){
