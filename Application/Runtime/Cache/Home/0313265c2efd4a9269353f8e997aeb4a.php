...
<article class="page-container">
	...
	<form action="/thinkphp_3.2.3_full/Home/Relation/relation_add" method="post"  enctype="multipart/form-data" >
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<tr class="text-c">
			<th width="40" >当前学生用户</th>
			<th width="40" >
			<input type="text" class="input-text" value="" placeholder=""  name="stu_name" id="stu_name">
			</th>
		</tr>
		<tr class="text-c">
			<th width="40">最高权角色名称</th>
			<th width="40">
            <select name="role_id" id="role_id">
            <?php if(is_array($role)): $i = 0; $__LIST__ = $role;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["role_id"]); ?>" selected><?php echo ($vo["role_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
			</th>
		</tr>
